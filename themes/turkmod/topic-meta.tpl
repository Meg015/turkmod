...
<div class="topic-info-row"><i class="bi {item.icon}" aria-hidden="true"></i><span>{item.label}</span><strong title="{item.value}" data-info-value tabindex="0">{if item.url}<a href="{item.url}">{item.value}</a>{else}{item.value}{/if}</strong></div>
{/loop}
</div>
</section>
</div>
