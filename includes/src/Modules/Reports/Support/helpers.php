...
{
    return topicReportService()->reasonLabels($GLOBALS['pdo'] ?? null);
}
...
function submitTopicReport(?PDO $pdo, int $topicId, int $reporterUserId, string $reason, string $details = '', array $reporter = []): array
{
    return topicReportService()->submit($pdo, $topicId, $reporterUserId, $reason, $details, $reporter);
}
...
{
    return topicReportService()->deleteAll($pdo, $actorId);
}
