...
return new class implements Migration
{
    ...
    {
        return '2026_06_21_0001_create_notification_module_tables';
    }

    public function up(PDO $pdo): void
    {
        $this->ensureNotificationEventColumns($pdo);
        $this->ensureTemplateTable($pdo);
        $this->ensureEmailQueueTable($pdo);
    }

    public function down(PDO $pdo): void
    {
        $pdo->exec('DROP TABLE IF EXISTS `notification_email_queue`');
        $pdo->exec('DROP TABLE IF EXISTS `notification_templates`');
        $this->dropNotificationEventColumns($pdo);
    }

    private function ensureNotificationEventColumns(PDO $pdo): void
    {
        ...
        foreach ($indexSql as $index => $sql) {
            if (!$this->indexExists($pdo, 'notifications', $index)) {
                $pdo->exec($sql);
            }
        }
    }

    private function ensureTemplateTable(PDO $pdo): void
    {
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS `notification_templates` (
                ...
                `email_link_template` VARCHAR(1024) NULL,
                `email_preview_template` VARCHAR(255) NULL,
    ...
    private function ensureEmailQueueTable(PDO $pdo): void
    {
        $pdo->exec("
    ...
    private function dropNotificationEventColumns(PDO $pdo): void
    {
        if (!$this->tableExists($pdo, 'notifications')) {
            return;
        }
        ...
        foreach ([
            ...
            'notifications_entity_index',
            'notifications_actor_index',
            'notifications_dedupe_index',
        ] as $index) {
            if ($this->indexExists($pdo, 'notifications', $index)) {
                $pdo->exec('ALTER TABLE `notifications` DROP INDEX `' . $index . '`');
            }
        }

        foreach (['delivery_channels', 'dedupe_key', 'actor_user_id', 'entity_id', 'entity_type', 'event_key'] as $column) {
            if (isset($this->columns($pdo, 'notifications')[$column])) {
                $pdo->exec('ALTER TABLE `notifications` DROP COLUMN `' . $column . '`');
            }
        }
    }
