...
if (!function_exists('eventsActivityHookDefinitions')) {
    function eventsActivityHookDefinitions(): array
    {
        return [
            ...
            'wheel_spin' => ['wired' => true, 'risk' => 'Dusuk', 'location' => 'includes/src/Modules/Events/Api/Endpoints/wheel-spin.php'],
            'topic_downloaded' => ['wired' => true, 'risk' => 'Orta', 'location' => 'includes/src/Engine/Topics/Support/helpers.php'],
            'user_registered' => ['wired' => true, 'risk' => 'Dusuk', 'location' => 'register.php'],
            'profile_avatar_uploaded' => ['wired' => true, 'risk' => 'Dusuk', 'location' => 'includes/src/Engine/Users/Support/profile-helpers.php'],
            'collection_created' => ['wired' => true, 'risk' => 'Dusuk', 'location' => 'includes/src/Engine/Topics/Support/helpers.php'],
            ...
            'user_reported' => ['wired' => false, 'risk' => 'Yuksek', 'location' => 'includes/src/Modules/Reports/Support/helpers.php'],
...
if (!function_exists('eventsEnsureGroupScopeSchema')) {
    function eventsEnsureGroupScopeSchema(PDO $pdo): void
    {
        ...
        if (!eventsColumnExists($pdo, 'events_tasks', 'group_id')) {
            $missing[] = 'events_tasks.group_id';
        }
        if (!eventsColumnExists($pdo, 'events_activity_rules', 'required_group_id')) {
            $missing[] = 'events_activity_rules.required_group_id';
        }
        if ($missing !== []) {
            throw new RuntimeException('Missing Events group scope schema: ' . implode(', ', $missing) . '; run Database Synchronization.');
        }
...
if (!function_exists('eventsProcessLoginStreak')) {
    function eventsProcessLoginStreak(PDO $pdo, int $userId, array $config): void
    {
        ...
        try {
            if (!eventsTableExists($pdo, 'events_user_streaks')) {
                throw new RuntimeException('Missing events_user_streaks; run Database Synchronization.');
            }
