        ...
        <div class="card-body ui-events-table-wrap ui-events-admin-table-wrap ui-panel__body ui-table-wrap ui-surface">
            ...
            <?php else: ?>
                ...
                <?php if ($totalRows > $perPage): ?>
                    <?php $queryString = '&action=' . urlencode($filterAction ?? 'all') . '&date=' . urlencode($filterDate ?? 'all'); ?>
                    <div class="ui-events-admin-pagination ui-events-audit-pagination">
                        <div class="ui-events-audit-pagination-summary">
                            Toplam <strong><?= $totalRows ?></strong> kayıt, <strong><?= $currentPage ?>.</strong> sayfa gösteriliyor.
                        </div>
                        <div class="ui-events-audit-pagination-actions">
                            <?php if ($currentPage > 1): ?>
                                <a href="?p=<?= $currentPage - 1 ?><?= $queryString ?>" class="ui-admin-btn ui-admin-btn-outline ui-admin-btn-sm"><i class="bi bi-chevron-left"></i> Önceki</a>
                            <?php else: ?>
                                <button class="ui-admin-btn ui-admin-btn-outline ui-admin-btn-sm" disabled><i class="bi bi-chevron-left"></i> Önceki</button>
                            <?php endif; ?>
                            
                            <?php if ($currentPage * $perPage < $totalRows): ?>
                                <a href="?p=<?= $currentPage + 1 ?><?= $queryString ?>" class="ui-admin-btn ui-admin-btn-outline ui-admin-btn-sm">Sonraki <i class="bi bi-chevron-right"></i></a>
                            <?php else: ?>
                                <button class="ui-admin-btn ui-admin-btn-outline ui-admin-btn-sm" disabled>Sonraki <i class="bi bi-chevron-right"></i></button>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
