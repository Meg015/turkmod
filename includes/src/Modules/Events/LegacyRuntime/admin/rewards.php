...
if ($ready && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        if ($action === 'manual_reward') {
            ...
            if ($userId <= 0) {
                $errors['user_id'] = 'Kullanıcı seçimi gerekli.';
            }
            ...
            if ($errors === []) {
                ...
                flash('success', 'Manuel ödül oluşturuldu.');
        ...
        // Ödül Kataloğu Kayıt/Güncelleme
        if ($action === 'save_item') {
            ...
            if (!$normalized['valid']) {
                ...
                flash('success', 'Ödül kataloğa kaydedildi.');
...
if ($ready) {
    try {
        ...
        $rewards = $pdo->query("SELECT ur.*, u.name AS user_name, u.email AS user_email FROM events_user_rewards ur LEFT JOIN users u ON u.id = ur.user_id ORDER BY ur.id DESC LIMIT 150")->fetchAll(PDO::FETCH_ASSOC) ?: [];
        $users = $pdo->query("SELECT id, name, email FROM users WHERE deleted_at IS NULL ORDER BY id DESC LIMIT 250")->fetchAll(PDO::FETCH_ASSOC) ?: [];
...
$renderItemForm = static function (array $item, string $panelKey, bool $isActive, string $baseUri): void {
    ...
    $displayTitle = $name !== '' ? $name : 'Yeni ödül';
    ...
    >
        ...
        <div class="ui-events-rule-detail-head ui-admin-detail-modal-head ui-panel__head">
            <div class="ui-events-rule-detail-titlebar">
                ...
                <div class="ui-events-rule-title-actions">
                    ...
                    <button class="ui-events-rule-modal-close ui-admin-detail-close" type="button" data-ui-events-item-close aria-label="Ayarları kapat"><i class="bi bi-x-lg"></i></button>
                </div>
            </div>

            <div class="ui-events-rule-detail-summary">
                <div class="ui-events-rule-summary-card is-primary ui-card"><span>Değer</span><strong><?= e((string)$item['value']) ?></strong></div>
                <div class="ui-events-rule-summary-card ui-card"><span>Kalan Stok</span><strong><?= trim((string)$item['remaining_quantity']) === '' ? 'Sınırsız' : (int)$item['remaining_quantity'] ?> / <?= (int)$item['quantity'] ?></strong></div>
        ...
        <div class="ui-events-rule-detail-body ui-admin-detail-modal-body ui-panel__body">
            ...
            <div class="ui-events-rule-form-grid ui-grid">
                <div class="ui-events-rule-field is-wide">
                    <label for="item-name-<?= e($panelKey) ?>">Ödül Adı</label>
                ...
                <div class="ui-events-rule-field">
                    <label for="item-type-<?= e($panelKey) ?>">Tür</label>
                ...
                <div class="ui-events-rule-field">
                    ...
                    <input id="item-remaining-<?= e($panelKey) ?>" class="ui-events-rule-soft-control" type="number" min="0" name="remaining_quantity" value="<?= trim((string)$item['remaining_quantity']) === '' ? '' : (int)$item['remaining_quantity'] ?>" placeholder="Stokla aynı" <?= eventsReadableNumberDataAttributes(['key' => 'remaining_quantity', 'type' => 'number', 'min' => 0, 'display' => ['zeroLabel' => 'stokla aynı']]) ?>>
                    <?= eventsRenderReadableNumberValue(trim((string)$item['remaining_quantity']) === '' ? '' : (int)$item['remaining_quantity'], ['key' => 'remaining_quantity', 'type' => 'number', 'min' => 0, 'display' => ['zeroLabel' => 'stokla aynı']]) ?>
                </div>
                <div class="ui-events-rule-field">
                    <label for="item-expires-<?= e($panelKey) ?>">Geçerlilik günü</label>
                    <input id="item-expires-<?= e($panelKey) ?>" class="ui-events-rule-soft-control" type="number" min="0" name="expires_in_days" value="<?= trim((string)$item['expires_in_days']) === '' ? '' : (int)$item['expires_in_days'] ?>" placeholder="Süresiz" <?= eventsReadableNumberDataAttributes(['key' => 'expires_in_days', 'type' => 'number', 'min' => 0]) ?>>
            ...
            <div class="ui-events-rule-form-grid ui-grid">
                <label class="ui-events-rule-scope-card ui-card" for="item-active-<?= e($panelKey) ?>">
                    <span><strong>Aktif</strong><em>Bu ödül çekilişlerde seçilebilir.</em></span>
            ...
            <div class="ui-events-rule-form-grid ui-grid">
                <div class="ui-events-rule-field is-wide">
                    <label for="item-desc-<?= e($panelKey) ?>">Açıklama</label>
        ...
        <div class="ui-events-settings-savebar ui-events-form-toolbar ui-events-rule-savebar ui-events-admin-savebar" data-ui-events-dirty-message="Bu ödülde kaydedilmemiş değişiklikler var.">
            <span data-ui-events-savebar-message>Değişiklik bu ödül için uygulanır.</span>
            <button class="ui-admin-btn ui-admin-btn-primary ui-events-rule-save" type="submit"><i class="bi bi-save"></i> Ödülü Kaydet</button>
        </div>
    </form>
    <?php
};
...
<div class="ui-events-admin-page ui-events-admin-console ui-section" data-ui-events-admin-page="rewards">
    ...
    eventsAdminPageHero(
        'Ödül Merkezi',
        'Çekilişlerde verilecek ödülleri katalogda oluşturun ve dağıtılan ödülleri takip edin.',
        'bi-gift'
    );
    ?>
    ...
    <div class="ui-events-tabs-shell ui-events-admin-tabs-shell ui-section" data-ui-events-tabs-root data-ui-events-admin-component="tabs">
        <div class="ui-events-app-nav ui-events-app-nav-minimal ui-events-admin-tablist" role="tablist">
            <a href="#ui-events-admin-tab-rewards-catalog" class="ui-events-nav-link ui-events-admin-tab <?= $activeTab === 'catalog' ? 'is-active' : '' ?>" role="tab" aria-selected="<?= $activeTab === 'catalog' ? 'true' : 'false' ?>" aria-controls="ui-events-admin-tab-rewards-catalog" data-ui-events-tab="catalog">
                <i class="bi bi-tags"></i> Ödül Kataloğu
            </a>
            ...
            <a href="#ui-events-admin-tab-rewards-distributed" class="ui-events-nav-link ui-events-admin-tab <?= $activeTab === 'distributed' ? 'is-active' : '' ?>" role="tab" aria-selected="<?= $activeTab === 'distributed' ? 'true' : 'false' ?>" aria-controls="ui-events-admin-tab-rewards-distributed" data-ui-events-tab="distributed">
                <i class="bi bi-box-seam"></i> Dağıtılan Ödüller
            </a>
            <span class="ui-events-nav-separator" aria-hidden="true"></span>
            <a href="#ui-events-admin-tab-rewards-manual" class="ui-events-nav-link ui-events-admin-tab <?= $activeTab === 'manual' ? 'is-active' : '' ?>" role="tab" aria-selected="<?= $activeTab === 'manual' ? 'true' : 'false' ?>" aria-controls="ui-events-admin-tab-rewards-manual" data-ui-events-tab="manual">
                <i class="bi bi-plus-circle"></i> Manuel Ödül Ver
            </a>
        </div>
        ...
        <div class="ui-events-panel-body ui-events-tabs-body ui-events-admin-tabs-body ui-panel__body ui-panel">
                ...
                <div class="admin-card ui-events-admin-panel ui-events-rules-master-shell ui-section ui-panel">
                    <div class="card-body ui-events-rules-master-body ui-events-admin-master-body ui-panel__body">
                        ...
                        <div class="ui-events-master-actionbar ui-events-admin-actionbar" data-ui-events-item-toolbar data-ui-events-admin-component="actionbar">
                            <button class="ui-admin-btn ui-admin-btn-primary" type="button" data-ui-events-item-new="new"><i class="bi bi-plus-lg"></i> Yeni ödül</button>
                        </div>
                        ...
                        <div class="ui-events-rules-master-layout ui-events-admin-master-layout ui-section" data-ui-events-item-master data-ui-events-admin-master data-ui-events-item-initial="<?= e($initialItemKey) ?>">
                            <div class="ui-events-rules-list-panel ui-events-admin-list-panel ui-panel" aria-label="Ödüller listesi">
                                ...
                                <?php if ($catalogItems === []): ?>
                                    <?php eventsAdminEmptyState('bi-tags', 'Ödül Bulunamadı', 'Katalogda henüz bir ödül oluşturmadınız.'); ?>
                                <?php else: ?>
                                    <?php foreach ($catalogItems as $index => $item): ?>
                                        ...
                                        >
                                            ...
                                            <span class="ui-events-rule-row-value"><?= trim((string)$item['remaining_quantity']) === '' ? 'Sınırsız' : ((int)$item['remaining_quantity'] . ' / ' . (int)$item['quantity']) ?></span>
            ...
            <div class="ui-events-tab-panel ui-events-admin-tab-panel <?= $activeTab === 'manual' ? 'is-active' : '' ?> ui-panel" id="ui-events-admin-tab-rewards-manual" role="tabpanel" data-ui-events-tab-panel="manual" <?= $activeTab === 'manual' ? '' : 'hidden' ?>>
                <div class="admin-card ui-events-admin-panel ui-events-admin-panel-spaced ui-panel">
                    <?php eventsAdminPanelHeader('bi-plus-circle', 'Manuel Ödül Ver', 'Belirli bir kullanıcı için anında ödül atayın.'); ?>
                    <div class="card-body ui-events-admin-panel-body ui-panel__body ui-panel">
                        <form method="post" action="<?= htmlspecialchars(rtrim($baseUri ?? '', '/') . '/admin/events-rewards.php?tab=manual') ?>">
                            ...
                            <div class="ui-events-rule-form-grid ui-grid">
                                <div class="ui-events-rule-field"><label>Kullanıcı</label><select class="ui-events-rule-soft-control" name="user_id" required><option value="">Seçin</option><?php foreach ($users as $user): ?><option value="<?= (int)$user['id'] ?>">#<?= (int)$user['id'] ?> <?= e((string)$user['name']) ?> · <?= e((string)$user['email']) ?></option><?php endforeach; ?></select></div>
                                <div class="ui-events-rule-field"><label>Ödül adı</label><input class="ui-events-rule-soft-control" name="reward_name" required></div>
                                <div class="ui-events-rule-field"><label>Tür</label><select class="ui-events-rule-soft-control" name="reward_type"><?php foreach (eventsAllowedRewardTypes() as $type): ?><option value="<?= e($type) ?>"><?= e($type) ?></option><?php endforeach; ?></select></div>
                                <div class="ui-events-rule-field"><label>Değer</label><input class="ui-events-rule-soft-control" name="reward_value" required></div>
                                <div class="ui-events-rule-field"><label>Geçerlilik günü</label><input class="ui-events-rule-soft-control" type="number" min="0" name="expires_in_days" placeholder="Süresiz" <?= eventsReadableNumberDataAttributes(['key' => 'expires_in_days', 'type' => 'number', 'min' => 0]) ?>><?= eventsRenderReadableNumberValue('', ['key' => 'expires_in_days', 'type' => 'number', 'min' => 0]) ?></div>
                            </div>
                            <div class="ui-events-settings-savebar ui-events-form-toolbar ui-events-admin-savebar"><span data-ui-events-savebar-message>Manuel ödül tek kullanıcı için oluşturulur.</span><button class="ui-admin-btn ui-admin-btn-primary" type="submit"><i class="bi bi-gift"></i> Ödül Oluştur</button></div>
                        </form>
                    </div>
                </div>
            </div>

            <!-- DAĞITILAN ÖDÜLLER SEKME -->
            <div class="ui-events-tab-panel ui-events-admin-tab-panel <?= $activeTab === 'distributed' ? 'is-active' : '' ?> ui-panel" id="ui-events-admin-tab-rewards-distributed" role="tabpanel" data-ui-events-tab-panel="distributed" <?= $activeTab === 'distributed' ? '' : 'hidden' ?>>
                <div class="admin-card ui-events-admin-panel ui-panel">
                    <?php eventsAdminPanelHeader('bi-gift', 'Dağıtılan Kullanıcı Ödülleri', 'Çekilişten, çarktan veya manuel kazanılmış tüm ödüller.'); ?>
                    <div class="card-body ui-events-table-wrap ui-events-admin-table-wrap ui-panel__body ui-table-wrap ui-surface">
                        <?php if ($rewards === []): ?>
                            <?php eventsAdminEmptyState('bi-gift', 'Dağıtılan Ödül Yok', 'Kullanıcılara henüz hiç ödül verilmedi.'); ?>
                        <?php else: ?>
                            ...
                            <table class="ui-events-table">
                                <thead><tr><th>Kullanıcı</th><th>Ödül</th><th>Kaynak</th><th>Durum</th><th>Tarih</th><th>İşlem</th></tr></thead>
                                <tbody>
                                            ...
                                            <div class="ui-events-action-dropdown">
                                                ...
                                                <div class="ui-events-action-dropdown-menu">
                                                    <?php if (eventsRewardCanBeAppliedStatus((string)$reward['status'])): ?>
                                                        ...
                                                        <form class="ui-events-inline-form" method="post" action="<?= htmlspecialchars(rtrim($baseUri ?? '', '/') . '/admin/events-rewards.php?tab=distributed') ?>" data-ui-events-confirm="Bu ödül kullanıcıya teslim edilecek. Devam edilsin mi?" data-ui-events-confirm-title="Ödül teslim edilsin mi?" data-ui-events-confirm-ok="<?= e($applyLabel) ?>" data-ui-events-confirm-tone="success"><?= csrf_field() ?><input type="hidden" name="_events_action" value="apply_reward"><input type="hidden" name="reward_id" value="<?= (int)$reward['id'] ?>"><input type="hidden" name="reward_type" value="<?= e((string)$reward['reward_type']) ?>"><button class="ui-events-action-dropdown-item" type="submit"><i class="bi bi-check2"></i> <?= e($applyLabel) ?></button></form>
                                                        <?php if ((string)$reward['status'] === 'pending'): ?>
                                                            <form class="ui-events-inline-form" method="post" action="<?= htmlspecialchars(rtrim($baseUri ?? '', '/') . '/admin/events-rewards.php?tab=distributed') ?>" data-ui-events-confirm="Bu bekleyen ödül iptal edilecek. Kullanıcıya teslim edilmeyecek." data-ui-events-confirm-title="Ödül iptal edilsin mi?" data-ui-events-confirm-ok="İptal Et" data-ui-events-confirm-tone="danger"><?= csrf_field() ?><input type="hidden" name="_events_action" value="cancel_reward"><input type="hidden" name="reward_id" value="<?= (int)$reward['id'] ?>"><button class="ui-events-action-dropdown-item is-danger" type="submit"><i class="bi bi-x"></i> İptal Et</button></form>
                                                        <?php endif; ?>
                                                    <?php else: ?>
