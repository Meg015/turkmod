...
function ensureScraperSchema(?PDO $pdo): void
{
    ...
    $requiredTables = ['bot_sites', 'bot_category_mappings', 'bot_jobs', 'bot_imports'];
    $missing = [];
    ...
    foreach (['translated_title', 'translated_content', 'author_topic', 'topic_version'] as $column) {
        if (!adminColumnExists($pdo, 'bot_imports', $column)) {
            $missing[] = 'bot_imports.' . $column;
        }
    }
...
function publishScraperImport(?PDO $pdo, int $importId, int $categoryId, string $publishStatus = 'draft')
{
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO topics (category_id, author_id, title, slug, author_topic, topic_version, topic_descriptions, status, created_at, published_at) VALUES (?,?,?,?,?,?,?, ?,NOW(),?)");
        $stmt->execute([
            $categoryId,
            $authorId,
            $title,
