...
require_once dirname(__DIR__, 2) . '/Media/Support/helpers.php';
...
function profileEnsureColumns(PDO $pdo): void
{
    ...
    // Schema is migration-owned; callers use this readiness hook.
}
...
function ensureTopicCollectionsTables(?PDO $pdo): void
{
    ...
    foreach (['topic_collections', 'topic_collection_items'] as $table) {
        if (function_exists('usersTableExists') && !usersTableExists($pdo, $table)) {
            throw new RuntimeException('Missing ' . $table . '; run Admin Panel > Database Synchronization.');
        }
    }
}
...
function profileUpdate(PDO $pdo, int $userId, array $data): void
{
    ...
    if ($username === '') {
        ...
        throw new RuntimeException("Kullanici adi {$bounds['min']}-{$bounds['max']} karakter olmali ve sadece harf, rakam, _ veya - icermelidir.");
    }
    if ($usernamePolicyError !== '') {
        throw new RuntimeException($usernamePolicyError);
    }
...
function profileCountReports(PDO $pdo, int $userId): int
{
    ...
    return profileCountRows($pdo, 'topic_reports', 'reporter_user_id', $userId);
}
