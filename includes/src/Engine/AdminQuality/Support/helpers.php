...
use App\Engine\Scraper\Support\ScraperUrlGuard;

require_once dirname(__DIR__, 2) . '/Scraper/Support/ScraperUrlGuard.php';
...
function adminQualityEnsureSchema(?PDO $pdo): void
{
    ...
    $required = [
        'topics' => ['moderation_flags', 'last_checked_at', 'health_status', 'health_summary'],
        'topic_download_links' => ['last_checked_at', 'last_status_code', 'health_status', 'last_health_message', 'last_final_url'],
        'media_files' => ['last_checked_at', 'last_status_code', 'health_status', 'last_health_message'],
    ];
    $missing = [];
    ...
    if ($missing !== []) {
        throw new RuntimeException('Missing content quality schema: ' . implode(', ', $missing) . '; run Database Synchronization.');
    }
}
...
function adminQualitySeoSummary(?PDO $pdo): array
{
    ...
    try {
        ...
        $summary['canonical_missing'] = trim((string) adminSettingValue($pdo, 'canonical_base_url', '')) === '' ? 1 : 0;
...
function adminQualityCheckTopicHealth(?PDO $pdo, int $topicId, int $externalLimit = 5): array
{
    ...
    adminQualityEnsureSchema($pdo);
    $stmt = $pdo->prepare("SELECT id, title, slug, topic_descriptions, primary_media_file_id
                           FROM topics
    ...
    if (function_exists('getTopicDownloadLinks')) {
        getTopicDownloadLinks($pdo, $topicId);
    }
